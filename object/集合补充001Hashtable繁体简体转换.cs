...
namespace 利用Hashtable繁体简体转换
{
    class program
    {
        static string strConvert(string jt, Hashtable dict)
        {
            ...
            {
                //判断词典中是否包含有翻译的字符
                if (!dict.ContainsKey(jt[i])) { Console.WriteLine("对不起，词典中不含有字符{0}", jt[i]); }
                else
                {
        ...
        static void Main(String[] args)
        {
            ...
            string ft = "我在垃圾上單廢了發見佛阿丟放假啊是放超級開心最姐艺看了辰快速的反擊群毆嗯誤入歧途假了啦盡快發了多少開飛機啊老師綠色的減肥啦神經康復來到房間啊來撒的風景";
            //建立词典
            Hashtable ht = new Hashtable();
            for (int i = 0; i < jt.Length; i++)
            {
                //如果有重复值，添加会出错，排查好久不知道什么原因。所以排除下
